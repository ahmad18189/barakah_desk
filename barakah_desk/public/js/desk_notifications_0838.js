(function () {
	...
	const format12hWithSeconds = (dateValue) => {
	...
	frappe.ui.toolbar.show_today_prayer_times = function () {
		...
		frappe.call({
			...
			callback(r) {
				...
				const clockId = `barakah-current-time-${frappe.utils.get_random(8)}`;
				...
				frappe.msgprint({
					...
					indicator: "blue",
					message: `
						<div class="barakah-current-time" style="margin-bottom: 12px; text-align: center;">
							<div style="color: var(--text-muted); font-size: 12px;">${__("Current Time")}</div>
							<div id="${clockId}" style="font-weight: 700; font-size: 20px;">${format12hWithSeconds(payload.server_now)}</div>
							<div style="color: var(--text-muted); font-size: 11px;">${payload.timezone || ""}</div>
						</div>
				...
				const updateClock = () => {
					...
					const now = new Date(serverNowMs + (Date.now() - clientStartMs));
					el.textContent = format12hWithSeconds(now);
				};
